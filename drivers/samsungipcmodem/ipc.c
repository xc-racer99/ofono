...
typedef void (*IpcSourceFunc) (struct ipc_message *resp,
                               gpointer user_data);
...
static gboolean ipc_watch_prepare(GSource *source, gint *timeout_)
{
	...
	*timeout_ = 0;
	...
	return ipc_client_poll(ipc_source->client, NULL, &timeval) > 0;
}
							...
							gpointer user_data)
{
	...
	while (ipc_client_poll(ipc_source->client, NULL, &timeval) > 0) {
...
static void handle_response(struct ipc_device *device, struct ipc_message *resp)
{
	...
	if (!ipc_seq_valid(resp->aseq)) {
		ofono_debug("Found response with invalid aseq");
		return;
	}
	...
	req = list->data;
	if (req->cb)
		req->cb(resp->command, resp->data, resp->size, req->cb_data);

	g_queue_delete_link(device->wait_queue, list);
}
...
static void received_fmt_data(struct ipc_message *resp, struct ipc_device *device)
{
	switch (resp->type) {
	case IPC_TYPE_RESP:
...
static void received_rfs_data(struct ipc_message *resp, struct ipc_device *device)
{
...
static guint add_response_watch(struct ipc_device *device, uint8_t type,
				uint16_t cmd, ipc_cb_func_t notify_cb,
				void *user_data)
{
...
static void ipc_device_send_message(struct ipc_device *device,
				    struct ipc_request *req)
{
	int ret;
	...
	ret = ipc_client_send(device->fmt_client, req->id, req->command,
			      req->type, req->data, req->length);
	...
	if (req->cb != NULL) {
		if (ret < 0) {
			ofono_debug("Sending request failed: %d", ret);
			req->cb(IPC_GEN_PHONE_RES, NULL, 0, req->cb);
		} else {
			g_queue_push_tail(device->wait_queue, req);
		}
	}

	ofono_debug("%d requests in the queue",
		    g_queue_get_length(device->wait_queue));
}

void ipc_device_enqueue_message(struct ipc_device *device,
			       uint16_t command, uint8_t type, void *data,
			       uint16_t length, ipc_cb_func_t cb,
			       void *user_data)
{
	struct ipc_request *req;
	...
	if (req == NULL) {
		if (cb != NULL)
			cb(IPC_GEN_PHONE_RES, NULL, 0, user_data);
		return;
	}
	...
	ipc_device_send_message(device, req);
}
...
void ipc_device_close(struct ipc_device *device)
{
	...
	if (device->rfs_watch > 0)
		g_source_remove(device->rfs_watch);
