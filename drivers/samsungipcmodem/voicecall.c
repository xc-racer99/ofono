			...
			ofono_voicecall_cb_t cb, void *data)
{
	...
	if(ipc_device_enqueue_message(vd->device, IPC_CALL_RELEASE, IPC_TYPE_EXEC,
						reqs, sizeof(struct ipc_call_cont_dtmf_data) * tone_count,
						send_dtmf_cb, cbd) > 0)
		return;
			...
			void *data)
{
	...
	if(ipc_device_enqueue_message(vd->device, IPC_CALL_OUTGOING, IPC_TYPE_EXEC,
						req, sizeof(struct ipc_call_outgoing_data),
...
static void call_list_cb(uint16_t cmd, void *data, uint16_t length, uint8_t error, void *user_data)
{
	...
	for (n = 0; n < num_entries; n++) {
		...
		call.id = entry->id;
		call.type = (entry->type == IPC_CALL_TYPE_VOICE || entry->type == IPC_CALL_TYPE_DEFAULT) ? 0 : 1;
		...
		if (entry_number != NULL) {
			...
			call.phone_number.type = (entry->number_length > 0 && call.phone_number.number[0] == '+') ? 145 : 129;
			g_free(entry_number);
		}
