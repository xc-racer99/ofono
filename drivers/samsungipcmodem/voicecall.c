			 ...
			 void *user_data)
{
	...
	/* TODO - should this be IPC_CALL_CONT_DTMF? */
	if (data == NULL) {
			...
			ofono_voicecall_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(vd->device, IPC_CALL_RELEASE,
			IPC_TYPE_EXEC, reqs,
				...
				ofono_voicecall_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(vd->device, IPC_CALL_ANSWER,
				   IPC_TYPE_EXEC, NULL, 0,
				   common_call_cb, cbd);
}
			...
			void *data)
{
	...
	ipc_device_enqueue_message(vd->device, IPC_CALL_OUTGOING,
				   IPC_TYPE_EXEC, req,
			 ...
			 void *user_data)
{
	...
	if (data == NULL || length < sizeof(struct ipc_call_list_header))
		return;
...
static void update_call_status(struct ofono_voicecall *vc)
{
	...
	ipc_device_enqueue_message(vd->device, IPC_CALL_LIST, IPC_TYPE_GET,
				   NULL, 0, call_list_cb, vc);
}
