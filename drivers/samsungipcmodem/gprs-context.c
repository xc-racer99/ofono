...
static void define_pdp_context_cb(uint16_t cmd, void *data, uint16_t length, uint8_t error, void *user_data)
{
	...
	ipc_gprs_pdp_context_request_set_setup(req, 1, /* activate the context */
	...
	if(ipc_device_enqueue_message(gcd->device, IPC_GPRS_DEFINE_PDP_CONTEXT, IPC_TYPE_SET,
						req, sizeof(struct ipc_gprs_pdp_context_request_set_data),
				...
				ofono_gprs_context_cb_t cb, void *data)
{
	...
	if(ipc_device_enqueue_message(gcd->device, IPC_GPRS_DEFINE_PDP_CONTEXT, IPC_TYPE_SET,
						req, sizeof(struct ipc_gprs_define_pdp_context_data),
					...
					ofono_gprs_context_cb_t cb, void *data)
{
	...
	ipc_gprs_pdp_context_request_set_setup(req, 0, /* deactivate the context */
					cid, NULL, NULL);
