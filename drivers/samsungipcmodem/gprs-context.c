					    ...
					    uint16_t length, void *user_data)
{
	...
	snprintf(gateway, IP_MAX_LENGTH, "%i.%i.%i.%i", resp->gateway[0],
		 resp->gateway[1], resp->gateway[2], resp->gateway[3]);
	snprintf(subnetmask, IP_MAX_LENGTH, "%i.%i.%i.%i", resp->subnet_mask[0],
				  ...
				  void *user_data)
{
	...
	ipc_device_enqueue_message(gcd->device, IPC_GPRS_PDP_CONTEXT,
			IPC_TYPE_SET, req,
			sizeof(struct ipc_gprs_pdp_context_request_set_data),
			pdp_context_activate_cb, cbd);
	return;
				...
				ofono_gprs_context_cb_t cb, void *data)
{
	...
	if (ipc_gprs_define_pdp_context_setup(req, 1, ctx->cid, ctx->apn) < 0)
		goto error;

	ipc_device_enqueue_message(gcd->device,
			IPC_GPRS_DEFINE_PDP_CONTEXT, IPC_TYPE_SET, req,
			sizeof(struct ipc_gprs_define_pdp_context_data),
			define_pdp_context_cb, cbd);
