				...
				ofono_devinfo_query_cb_t cb, void *user_data)
{
	...
	if (ipc_device_enqueue_message(data->device, IPC_MISC_ME_VERSION,
				       IPC_TYPE_GET, msg, 1,
				       misc_me_version_cb, cbd) > 0)
		return;
				...
				ofono_devinfo_query_cb_t cb, void *user_data)
{
	...
	if (ipc_device_enqueue_message(data->device, IPC_MISC_ME_SN,
				       IPC_TYPE_GET, msg, 1, misc_me_sn_cb,
				       cbd) > 0)
		return;
