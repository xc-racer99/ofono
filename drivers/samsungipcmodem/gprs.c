...
static void gprs_attached_status_cb(uint16_t cmd, void *data, uint16_t length,
				    void *user_data)
{
	...
	if (data == NULL || length < sizeof(struct ipc_net_regist_response_data)) {
				...
				ofono_gprs_status_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(gd->device, IPC_NET_REGIST,
				   IPC_TYPE_GET, req,
				   sizeof(struct ipc_net_regist_request_data),
				   gprs_attached_status_cb, cbd);
}
...
static void gprs_status_cb(uint16_t cmd, void *data, uint16_t length,
			   void *user_data)
{
	struct ofono_gprs *gprs = user_data;
	struct ipc_net_regist_response_data *resp = data;

	if (data == NULL || resp->domain != IPC_NET_SERVICE_DOMAIN_GPRS)
		return;

	ofono_gprs_status_notify(gprs, resp->status - 1);
}

static void notify_hsdpa_status_cb(uint16_t cmd, void *data, uint16_t length,
				   void *user_data)
{
	struct ofono_gprs *gprs = user_data;
	struct gprs_data *gd = ofono_gprs_get_data(gprs);
			      ...
			      void *user_data)
{
	...
	gd->status_watch = ipc_device_add_notification_watch(gd->device,
							IPC_NET_REGIST,
							notify_gprs_status_cb,
							gprs);
}
				...
				unsigned int vendor, void *user_data)
{
	...
	gd->status_watch = ipc_device_add_notification_watch(gd->device,
							    ...
							    initial_status_cb,
							    gprs);
	gd->hsdpa_status_watch = ipc_device_add_notification_watch(gd->device,
							IPC_GPRS_HSDPA_STATUS,
							notify_hsdpa_status_cb,
							gprs);

	client = ipc_device_get_fmt_client(gd->device);
