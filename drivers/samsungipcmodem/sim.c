				...
				void *user_data)
{
	...
	if (data == NULL || length < sizeof(struct ipc_sec_phone_lock_response_data)) {
				...
				ofono_query_facility_lock_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(sd->device, IPC_SEC_PHONE_LOCK,
			IPC_TYPE_GET, req,
			sizeof(struct ipc_sec_phone_lock_request_get_data),
			pin_query_locked_cb, cbd);
}
			     ...
			     void *user_data)
{
	...
	if (data == NULL || cmd != IPC_GEN_PHONE_RES ||
				...
				ofono_sim_lock_unlock_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(sd->device, IPC_SEC_CHANGE_LOCKING_PW,
				IPC_TYPE_SET, req,
				sizeof(struct ipc_sec_change_locking_pw_data),
				change_passwd_cb, cbd);
}
			   ...
			   void *user_data)
{
	...
	if (data == NULL ||
			length < sizeof(struct ipc_sec_rsim_access_response_header) ||
			cmd != IPC_SEC_RSIM_ACCESS) {
					...
					ofono_sim_read_cb_t cb, void *cb_data)
{
	...
	ipc_device_enqueue_message(sd->device, IPC_SEC_RSIM_ACCESS,
			IPC_TYPE_GET, req,
				 ...
				 void *user_data)
{
	...
	if (data == NULL ||
			length < sizeof(struct ipc_sec_pin_status_response_data)) {
		CALLBACK_WITH_FAILURE(cb, -1, cbd->data);
		goto done;
	}
			 ...
			 void *user_data)
{
	...
	if (data == NULL || length < sizeof(struct ipc_misc_me_imsi_header)) {
				...
				ofono_sim_imsi_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(sd->device, IPC_MISC_ME_IMSI,
				       IPC_TYPE_GET, NULL, 0,
				       read_imsi_cb, cbd);
}
			 ...
			 void *user_data)
{
	...
	case IPC_SEC_PIN_TYPE_PIN1:
		...
		ipc_device_enqueue_message(sd->device,
					 ...
					 void *data)
{
	...
	ipc_device_enqueue_message(sd->device, IPC_SEC_LOCK_INFOMATION,
			IPC_TYPE_GET, req,
			sizeof(struct ipc_sec_lock_infomation_request_data),
			lock_info_cb, cbd);
}
				  ...
				  void *user_data)
{
	...
	struct ipc_sec_pin_status_response_data *resp;
	...
	/* If IPC_GEN_PHONE_RES is the return type, then there's no SIM */
	if (cmd == IPC_GEN_PHONE_RES) {
