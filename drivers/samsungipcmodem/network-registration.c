...
static void list_operators_cb(uint16_t cmd, void *data, uint16_t length,
			      void *user_data)
{
	...
	struct ipc_net_plmn_list_entry *entries = data +
				sizeof(struct ipc_net_plmn_list_header);
	int i;

	DBG("Got %d elements", resp->count);

	if (data == NULL || length < sizeof(struct ipc_net_plmn_list_header)) {
				...
				ofono_netreg_operator_list_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(nd->device, IPC_NET_PLMN_LIST,
				   IPC_TYPE_GET, NULL, 0,
				   list_operators_cb, cbd);
}
...
static void network_register_cb(uint16_t cmd, void *data, uint16_t length,
				void *user_data)
{
	struct cb_data *cbd = user_data;
	...
	if (data == NULL || ipc_gen_phone_res_check(resp) < 0) {
				...
				ofono_netreg_register_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(nd->device, IPC_NET_PLMN_SEL,
				IPC_TYPE_SET, req,
				sizeof(struct ipc_net_plmn_sel_request_data),
				network_register_cb, cbd);
}
...
static void current_operator_cb(uint16_t cmd, void *data, uint16_t length,
				void *user_data)
{
	struct cb_data *cbd = user_data;
	...
	if (data == NULL || length < sizeof(struct ipc_net_serving_network_data)) {
	...
	DBG("%s: mcc=%s, mnc=%s, reg_state=%d, tech=%d", __func__,
				...
				ofono_netreg_operator_cb_t cb, void *data)
{
	...
	ipc_device_enqueue_message(nd->device, IPC_NET_SERVING_NETWORK,
				   IPC_TYPE_GET, NULL, 0,
				   current_operator_cb, cbd);
}

static void net_status_cb(uint16_t cmd, void *data, uint16_t length,
			  void *user_data)
{
	struct cb_data *cbd = user_data;
	...
	if (data == NULL ||
			length < sizeof(struct ipc_net_regist_response_data)) {
					...
					void *data)
{
	...
	ipc_device_enqueue_message(nd->device, IPC_NET_REGIST,
				   IPC_TYPE_GET, req,
				   sizeof(struct ipc_net_regist_request_data),
				   net_status_cb, cbd);
}

static void notify_status_cb(uint16_t cmd, void *data, uint16_t length,
			     void *user_data)
{
...
static void notify_rssi_cb(uint16_t cmd, void *data, uint16_t length,
			   void *user_data)
{
	struct ofono_netreg *netreg = user_data;
	struct netreg_data *nd = ofono_netreg_get_data(netreg);
...
static void initial_status_cb(uint16_t cmd, void *data, uint16_t length,
			      void *user_data)
{
	struct ofono_netreg *netreg = user_data;
	struct ipc_net_regist_response_data *resp = data;
	...
	nd->status_watch = ipc_device_add_notification_watch(nd->device,
							    IPC_NET_REGIST,
							    notify_status_cb,
							    netreg);

	nd->rssi_watch = ipc_device_add_notification_watch(nd->device,
							  IPC_DISP_RSSI_INFO,
							  notify_rssi_cb,
							  netreg);
}

static int samsungipc_netreg_probe(struct ofono_netreg *netreg,
